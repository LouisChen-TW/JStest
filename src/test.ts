...
function countEmployees(factoriesArr: Factory[]): object[] {
...
function countFactories(factoriesArr: Factory[]): any {
  const result = []
  const existEmployee: any = {}
...
function orderByAlphabetical(factoriesArr: Factory[]) {
  const result = factoriesArr.map(factory => {
    return {
      name: factory.name,
...
const tasks = [
  { id: 1, title: 'task01', duration: 60 }, // 1
  { id: 2, title: 'task02', duration: 120 }, // 2
  { id: 3, title: 'task03', duration: 180 }, // 3
  { id: 4, title: 'task04', duration: 360 }, // 6
  { id: 5, title: 'task05', duration: 30 }, // 30/60
  { id: 6, title: 'task06', duration: 220 }, // 3 40/60
  { id: 7, title: 'task07', duration: 640 }, // 10 40/60
  { id: 8, title: 'task08', duration: 250 }, // 4 10/60
  { id: 9, title: 'task09', duration: 119 }, // 1 59/60
  { id: 10, title: 'task10', duration: 560 }, // 9 20/60
  { id: 11, title: 'task11', duration: 340 }, // 5 40/60
  { id: 12, title: 'task12', duration: 45 }, // 45/60
  { id: 13, title: 'task13', duration: 86 }, // 1 26/60
  { id: 14, title: 'task14', duration: 480 }, // 8
...
function calculateDuration(workBegin: string, workEnd: string): number {
...
function countTotalHoursWorked(
  employeeType: WorkingType[],
  employees: Employee[]
): number {
  let totalHoursWorked = 0
  for (const employee of employees) {
    const workBegin = (
      employeeType.find(type => type.id === employee.type) as WorkingType
    ).work_begin
    const workEnd = (
      employeeType.find(type => type.id === employee.type) as WorkingType
    ).work_end
...
function howManyEmployeeByTime(time: string): number {
...
function howManyDaysForTasks(tasks: Task[]) {
